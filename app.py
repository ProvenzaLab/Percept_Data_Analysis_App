...
class Worker(QObject):
    ...
    def run(self):
        try:
            self.param_dict = gui_utils.translate_param_dict(read_json('param.json'))
            save_to_json(self.param_dict, 'param.json')
    ...
    def continue_processing(self):
        try:
            percept_data, zone_index = generate_data.generate_data(
                subject_name=self.param_dict['subject_name'], file_list=self.file_list
            )
            percept_data = calc_circadian.calc_circadian(
                percept_data=percept_data, zone_index=zone_index
            )
...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Percept Data App v_1.0")
        self.setGeometry(100, 100, 800, 600)
        self.data = {
            "dbs_date": "12-22-2023",
            "responder_zone_idx": "",
            "non_responder_zone_idx": "",
            "subject_name": "009",
    ...
    def show_frame2(self):
        ...
        self.frame2 = Frame2(self, self.data, self.worker.percept_data, self.worker.param_dict, self.worker.zone_index)
...
class Frame2(QWidget):
    def __init__(self, parent, data, percept_data, param_dict, zone_index):
        super().__init__(parent)
        self.parent = parent
        self.data = data
        self.percept_data = percept_data
    ...
    def initUI(self):
        ...
        self.export_button = QPushButton("Export LinAR R2 feature", self)
        ...
        self.download_button.setIcon(QIcon("downloading.png"))
    ...
    def download_image(self):
        ...
        if file_path:
            file_path = gui_utils.add_extension(file_path, ".png")
            self.generate_plot_png(file_path)

    def export_data(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Data", "", "CSV Files (*.csv);;All Files (*)")
        if file_path:
            file_path = gui_utils.add_extension(file_path, ".csv")
            ...
            lin_ar_df.to_csv(file_path, mode='w', header=True, index=False)
    ...
    def save_data(self):
        ...
        for key, entry in self.entries.items():
            if isinstance(entry, tuple):
                self.parent.data[key] = [e.text() for e in entry]
            else:
                self.parent.data[key] = entry.text()

        self.parent.data["responder"] = self.responder_yes_checkbox.isChecked()
        if self.responder_yes_checkbox.isChecked():
            self.parent.data["responder_date"] = self.responder_date_entry.text()
        else:
            self.parent.data["responder_date"] = ""

        save_to_json(self.parent.data, 'param.json')
        self.parent.show_loading_screen()
