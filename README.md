...
- `src/pipeline.py`: Orchestrates the pipeline below (`generate_raw` → `process_data` → `model_data`) across every patient and pushes the combined result back to the GUI worker process.
- `src/generate_raw.py`: Generates a pandas DataFrame from Medtronic Percept json data files, which is used in subsequent analyses.
- `src/process_data.py`: Processes the raw DataFrame to clean, normalize, and z-score. Different artifact removing techniques are implemented and can be specified in the app settings.
- `src/model_data.py`: Analyzes the processed data DataFrame to perform AR(1)/AR(k) predictions and calculate predictability metrics (r2, residual variance, etc.).
- `utils/plotting_utils.py`: Generates a summary plot encapsulating the most critical information from the processed data.
- `utils/utils.py`: Provides various general helper methods utilized by the above files (outlier filling, interpolation, z-scoring).
- `utils/json_utils.py`: Provides helper methods to process the Medtronic Percept json data files.
- `utils/state_utils.py`: Provides helper methods to determine clinical states from patient dates.
- `utils/model_utils.py`: Provides helper methods to apply the autoregression model to the data.
...
1. `src/generate_raw.py`
2. `src/process_data.py`
3. `src/model_data.py`

`src/pipeline.py` wraps this sequence in `run_pipeline()`, which is what both the GUI worker process and the terminal runner call.

For a simple example of running this execution pipeline, refer to `terminal_runner.py`. This file provides a basic script to run the data analysis and export the results. To modify the hyperparameters, update them in `data/patient_info.json` and `data/param.json`.

For specific implementation details, refer to the documentation and comments within these scripts.

### GUI Interface

The GUI Interface lives in `src/ui/`, with each screen in its own module:

- `app.py` / `app_win.py`: Entry points that create the `QApplication`, apply the app-wide theme, and launch the main window. `app_win.py` additionally sets the Windows `AppUserModelID` and forwards to `app.main()`.
- `src/ui/main_window.py`: The `MainWindow` shell that hosts and swaps between all screens, starts the pipeline worker process, and routes results to the plots view. Also defines the `LoadingScreen`.
- `src/ui/opening.py`: The landing screen (`OpeningScreen`) with the primary actions and toolbar, plus the `HelpMenu` and `DocMenu` screens.
- `src/ui/patient.py`: The patient database screen (`PatientMenu`) for adding, listing, and deleting patients.
- `src/ui/settings.py`: The settings screen (`SettingsMenu`) that edits `data/param.json`.
- `src/ui/plots.py`: The results viewer (`Plots`) that renders the Plotly figure and exposes the export buttons.
- `src/ui/theme.py`: The single source of truth for app styling — one dark-theme stylesheet applied globally so every screen and dialog stays consistent.
- `utils/gui_utils.py`: A utility file used by the UI modules to perform tasks such as data export, validation, and file dialogs.
